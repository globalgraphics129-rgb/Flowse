...
import { RecurringTransaction, EXPENSE_CATEGORIES, INCOME_CATEGORIES } from '../types';
import CategoryIcon from './CategoryIcon';

interface RecurringFormProps {
  onAdd: (recurringData: Omit<RecurringTransaction, 'id' | 'isActive'>) => void;
  onClose: () => void;
  currencySymbol?: string;
}

export default function RecurringForm({ onAdd, onClose, currencySymbol = '₦' }: RecurringFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!description.trim()) {
      setErrorMsg('Please enter a title for the repeating schedule');
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center space-x-2.5">
            ...
            <h2 className="text-xl font-bold text-natural-text tracking-tight">New Recurring Transaction</h2>
          </div>
          <button
            id="recurring-form-close"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Interval frequency select dropdown */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-sage uppercase tracking-widest block pl-1">Frequency</label>
              <select
                id="recurring-frequency-select"
                className="w-full bg-natural border border-border-soft rounded-2xl py-3 px-4 text-natural-text text-xs font-bold focus:outline-none focus:border-sage transition-colors cursor-pointer"
                value={frequency}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Start date */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-sage uppercase tracking-widest block pl-1">Start Date</label>
              <div className="relative">
            ...
            {/* Description */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-sage uppercase tracking-widest block pl-1">Description Title</label>
              <input
                id="recurring-desc-input"
                type="text"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-sage uppercase tracking-widest block pl-1 font-semibold">Select Category</label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {activeCategories.map((cat) => {
                const isSelected = category === cat.value;
                return (
                  <button
                    id={`rec-cat-select-${cat.value}`}
                    key={cat.value}
                    type="button"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                placeholder="Include repeating note reference..."
                value={notes}
